...
- **Takrorlangan haqiqat yo'q:** denormallashtirilgan hisoblagichlar (o'qilmaganlar, a'zolar soni) — _hosila keshlar_, aniq belgilangan va qayta tiklanadigan — hech qachon manba emas.
...
`id`, `question_id` FK ON DELETE CASCADE, `label` text, `is_correct` boolean, `position` int. `INDEX (question_id)`. _(To'g'rilik bu yerda, faqat server tomonida — test paytida o'quvchiga hech qachon yuborilmaydi.)_
